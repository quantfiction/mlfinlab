...
class BaseBars(ABC):
        ...
        for batch in pd.read_csv(self.file_path, chunksize=self.batch_size):
            ...
            if to_csv is True:
                ...
                if final_bars is not None:
                    final_bars = np.r_[final_bars, arr_bars]
                else:
                    final_bars = arr_bars
            count += 1
    ...
    def _assert_csv(test_batch):
        ...
        assert not isinstance(
            test_batch.iloc[0, 2], str), 'volume column in csv not int or float.'
    ...
    def _create_bars(self, date_time, price, high_price, low_price):
        """
        ...
        # Update bars
        return np.array([date_time, open_price, high_price, low_price, close_price, volume])
